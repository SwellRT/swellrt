...
public class DeltaBasedEventSource implements Subscriber {
  ...
  /**
   * Evaluates path expresions defined as keys in the provided context data map
   * against the provided sub path.
   *
   * Example:
   *
   * if
   *
   * subPath = root.list.3
   * contextData["root.list.?.field"] = null
   *
   * then
   *
   * newContextData["root.list.?.field"] = (value of root.list.3.field)
   *
   * Returns a copy of the provided context data but including these new values.
   *
   */
  protected Map<String, String> evaluateContextData(UnmutableModel dataModel, String subPath,
      Map<String, String> contextData) {

    Map<String, String> newContextData = new HashMap<String, String>(contextData);

    String pathPrefix = subPath.substring(0, subPath.lastIndexOf(".")) + ".?";
    for (String key : contextData.keySet()) {
      if (key.startsWith(pathPrefix)) {
        String newKey = key.replace(pathPrefix, subPath);
        ReadableType value = dataModel.fromPath(newKey);
