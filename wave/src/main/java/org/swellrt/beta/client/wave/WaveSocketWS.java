...
public class WaveSocketWS implements WaveSocket {
  ...
  private final Scheduler.IncrementalTask heartbeatTask = new Scheduler.IncrementalTask() {
    ...
    public boolean execute() {
        if (ws.readyState == WebSocket.OPEN) {
          ...
          heartbeatAck = false;
          SchedulerInstance.getLowPriorityTimer().scheduleDelayed(hearbeatCheckTask, HEARTBEAT_TIMEOUT);
        } else {
          return false;
        }
        return true;
  ...
  protected void startHeartbeat() {
    if (heartbeatOn)
      SchedulerInstance.getLowPriorityTimer().scheduleRepeating(heartbeatTask, HEARTBEAT_INTERVAL, HEARTBEAT_INTERVAL);
  }
  ...
  protected void handleReconnectionMessage(String message) {
    ...
    try {
      ...
      Console.log("Error processing reconnection message: "+ex.getMessage());
    }       
  }
  ...
  protected void handleHeartbeatMessage(String message) {
    ...
    try {
      ...
      Console.log("Error processing heart beat message: "+ex.getMessage());
    }
